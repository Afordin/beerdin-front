		...
		const fetchContributors = async () => {
			try {
				const responseFront = await fetch(
					"https://api.github.com/repos/Afordin/beerdin-front/contributors",
				);
				const data1 = await responseFront.json();
				const responseBack = await fetch(
					"https://api.github.com/repos/afordin/beerdin/contributors",
				);
				const data2 = await responseBack.json();
				const responseAPI = await fetch(
					"https://api.github.com/repos/afordin/beerdin-api/contributors",
				);
				...
				const filteredResponse = contributorsData.reduce<
				...
				const sortedContributors = Object.values(filteredResponse).sort(
