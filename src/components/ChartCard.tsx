...
export const ChartCard = ({ chartCardData }: ChartCardProps) => {
	...
	const fetchData = async () => {
		...
		setResponse(chartCardData.urlEndpoint === "test");

		// Simula la carga de datos específicos
		switch (chartCardData.chartType) {
	...
	useEffect(() => {
		fetchData();
	}, [chartCardData.chartType, chartCardData.urlEndpoint]);
		...
		<Card className=" flex flex-col rounded-lg p-4 px-6 xl:p-12  ">
			...
			<CardContent className="p-0 flex justify-center items-center h-full pt-6">
				<Suspense fallback={<ChartSkeleton />}>
					{isLoading ? (
						<div
							className={` flex w-full h-full skeleton-fade ${!isLoading ? "hidden" : ""}`}
						>
							<ChartSkeleton />
						</div>
					) : (
						<div className="flex w-full h-full content-fade">
							{!response && (
							...
							{response && chartCardData.chartType === ChartType.BarList && (
								<div className="w-full">
									<div className="space</div>-x-2 text-left pb-6 pt-3">
